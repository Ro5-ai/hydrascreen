...
from hydrascreen.api import APIError, inference, upload_pdb, upload_sdf
...
def test_inference_successful_response(inference_request, mock_inference_pairs):
    ...
    result = inference(inference_pairs=mock_inference_pairs, token="test_token")

    assert result.equals(pd.read_csv(StringIO(response_str)))


@patch("hydrascreen.api.requests.post")
def test_inference_error_response(inference_request, mock_inference_pairs):
    mock_response = Mock()
    ...
    with pytest.raises(APIError):
        inference(inference_pairs=mock_inference_pairs, token="test_token")


@patch("hydrascreen.api.requests.post")
def test_upload_pdb_successful_response(upload_pdb_request, tmp_path):
    ...
    result = upload_pdb(pdb_file=open(mock_pdb_file, "rb"), token="test_token")

    assert result == "s3://test-bucket/protein.pdb"


@patch("hydrascreen.api.requests.post")
def test_upload_pdb_error_response(upload_pdb_request, tmp_path):
    mock_pdb_file = tmp_path / "protein.pdb"
    mock_pdb_file.touch()
    mock_response = Mock()
    mock_response.status_code = 500
    ...
    with pytest.raises(APIError):
        upload_pdb(pdb_file=open(mock_pdb_file, "rb"), token="test_token")


@patch("hydrascreen.api.requests.post")
def test_upload_sdf_successful_response(upload_sdf_request, tmp_path):
...
@patch("hydrascreen.api.requests.post")
def test_upload_sdf_error_response(upload_sdf_request, tmp_path):
    mock_sdf_file = tmp_path / "docked_ligand.sdf"
    mock_sdf_file.touch()
    mock_response = Mock()
    mock_response.status_code = 500
